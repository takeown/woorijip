...
class StoredValueAccountRepository(
    ...
    fun ensureDefaults(householdId: Long) {
        jdbcTemplate.update(
            """
            INSERT INTO stored_value_accounts (household_id, type, name)
            VALUES (:householdId, 'ONNURI_GIFT_CERTIFICATE', '온누리상품권'),
                   (:householdId, 'PREGNANCY_VOUCHER', '임산부 바우처')
    ...
    fun findAllByHouseholdId(householdId: Long): List<StoredValueAccount> =
        jdbcTemplate.query(
            """
            SELECT a.id, a.household_id, a.type, a.name, a.created_at,
