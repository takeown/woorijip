  ...
  return (
    <section className="mb-7 mt-5 rounded-2xl bg-stone-50 p-4">
      ...
      <details className="mt-3 rounded-xl border border-dashed border-emerald-300 bg-emerald-50/50">
        <summary className="flex min-h-11 cursor-pointer list-none items-center px-4 py-3 text-sm font-medium text-emerald-800 [&::-webkit-details-marker]:hidden">
          상품권·바우처 추가
        </summary>
        <form className="grid gap-3 border-t border-emerald-100 px-4 pb-4 pt-3" onSubmit={createAccount}>
          <label className="text-xs font-medium text-stone-600">
            이름
            <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" maxLength={100} name="name" placeholder="예: 서울사랑상품권" required />
          </label>
          <label className="text-xs font-medium text-stone-600">
            소유자
            <select className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-base lg:text-sm" name="ownerUserId" required>
              {householdMembers.map((member) => <option key={member.userId} value={member.userId}>{member.displayName}</option>)}
            </select>
          </label>
          <label className="text-xs font-medium text-stone-600">
            종류
            <select className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-base lg:text-sm" name="preset" onChange={(event) => setCreatePreset(event.target.value as AccountPreset)} value={createPreset}>
          ...
          {createPreset === "OTHER" ? (
            <label className="text-xs font-medium text-stone-600">
              종류명
              <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" maxLength={40} name="customCategoryName" placeholder="예: 육아 지원금" required />
            </label>
          ) : null}
          <button className="min-h-11 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" disabled={savingKey !== null} type="submit">
        ...
        {accounts.map((account) => (
          <details className="group overflow-hidden rounded-xl border border-stone-200 bg-white" key={account.id}>
            ...
            {!account.archived ? (
              <>
                <form className="grid gap-3 border-t border-stone-100 px-4 pb-4 pt-3" onSubmit={(event) => credit(event, account)}>
                  <label className="text-xs font-medium text-stone-600">
                    잔액 추가 금액
                    <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" min="1" name="balanceAmount" required type="number" />
                  </label>
                  <label className="text-xs font-medium text-stone-600">
                    실제 출금액
                    <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" defaultValue={account.category === "VOUCHER" ? 0 : undefined} min="0" name="paidAmount" required type="number" />
                  </label>
                  <label className="text-xs font-medium text-stone-600">
                    출금 계좌·지급처 <span className="font-normal">(선택)</span>
                    <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" maxLength={100} name="sourceName" />
                  </label>
                  <label className="text-xs font-medium text-stone-600">
                    일시
                    <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" name="occurredAt" required type="datetime-local" />
                  </label>
                  <button className="min-h-11 rounded-lg bg-stone-800 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" disabled={savingKey !== null} type="submit">
                    {savingKey === `credit-${account.id}` ? "저장 중..." : "잔액 추가"}
                  </button>
                </form>

                <details className="border-t border-stone-100 bg-amber-50/50">
                  <summary className="flex min-h-11 cursor-pointer list-none items-center px-4 py-3 text-sm font-medium text-amber-800 [&::-webkit-details-marker]:hidden">잔액 조정</summary>
                  <form className="grid gap-3 border-t border-amber-100 px-4 pb-4 pt-3" onSubmit={(event) => adjust(event, account)}>
                    ...
                    <label className="text-xs font-medium text-stone-600">
                      조정 방향
                      <select className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-base lg:text-sm" defaultValue="DECREASE" name="direction">
                    ...
                    <label className="text-xs font-medium text-stone-600">
                      조정 금액
                      <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" min="1" name="amount" required type="number" />
                    </label>
                    <label className="text-xs font-medium text-stone-600">
                      조정 사유
                      <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" maxLength={100} name="reason" placeholder="예: 누락 사용, 환불, 잔액 정정" required />
                    </label>
                    <label className="text-xs font-medium text-stone-600">
                      조정 일시
                      <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" name="occurredAt" required type="datetime-local" />
                    </label>
                    <button className="min-h-11 rounded-lg bg-amber-700 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" disabled={savingKey !== null} type="submit">
            ...
            <form className="grid gap-3 border-t border-stone-100 bg-stone-50 px-4 pb-4 pt-3" onSubmit={(event) => saveDetails(event, account)}>
              <label className="text-xs font-medium text-stone-600">
                이름
                <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" defaultValue={account.name} maxLength={100} name="name" required />
              </label>
              <label className="text-xs font-medium text-stone-600">
                분류
                <select className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-base disabled:bg-stone-100 lg:text-sm" disabled={account.automationKey !== null} name="category" onChange={(event) => setEditCategories((current) => ({ ...current, [account.id]: event.target.value as StoredValueAccountCategory }))} value={editCategories[account.id] ?? account.category}>
              ...
              {(editCategories[account.id] ?? account.category) === "OTHER" ? (
                <label className="text-xs font-medium text-stone-600">
                  종류명
                  <input className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base lg:text-sm" defaultValue={account.customCategoryName ?? ""} maxLength={40} name="customCategoryName" required />
                </label>
              ) : null}
              <div className="grid grid-cols-2 gap-2">
                <button className="min-h-11 rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-700 disabled:opacity-50" disabled={savingKey !== null} type="submit">정보 저장</button>
              ...
              {account.canDelete ? <button className="min-h-11 rounded-lg bg-red-700 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" disabled={savingKey !== null} onClick={() => deleteAccount(account)} type="button">삭제</button> : <p className="text-xs text-stone-500">사용 이력이 있어 삭제 대신 보관할 수 있습니다.</p>}
            </form>
          </details>
        ))}
      </div>
