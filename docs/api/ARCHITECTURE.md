...
결제자, 결제수단, 카테고리와 태그를 시간순으로 모두 반환한다. 웹의 기존 거래 수정 폼을
바로 열 수 있도록 결제자 ID, 분류·결제 원본 값, 잔액 계정과 생성·수정 시각도 함께 반환한다.
...
`payer`는 결제자를, `q`는 가맹점명과 내역을, `from`과 `to`는 서울 날짜 기준 포함 범위를
필터링한다. 다음 페이지에서도 같은 필터를 보내며 필터가 달라지면 커서를 처음부터 다시
시작한다.
...
거래 목록은 household, 결제자, 선택적 카테고리, 검색어, 날짜 범위와 커서 조건을 하나의 명시적 SQL로
조합한다. Service가 종료일을 서울 시간 기준 다음 날 0시의 미만 조건으로 변환하고,
검색어의 `%`, `_`와 `\`는 와일드카드가 아닌 일반 문자로 처리한다.
